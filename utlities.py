...
def graph_creation(n, plcs=True):
    # Edge devices
    ...
    node_list = [
        Laptop, HDD_Recorder, Portable_GD, Audio_device, 
        Security_Cam, AGW, SW1, SW2, SW3, SW4, WL1, WR]
    ...
    edge_list = [(AGW, WR), (AGW, SW1), (WR, Laptop), (SW1, Security_Cam),
                 ...
                 (WL1, Audio_device), (WL1, Portable_GD), (SW3, SW4), (SW4, Desktop_PC)]

    #Remove PLC stuff if plcs == True
    if plcs:
        extra_edges = [(PLC1, PLC4), (PLC4, PLC2), (SW3, PLC1),
                 (PLC4, PLC3), (PLC2, Air_condition), (PLC3, TV),
                 (PLC3, Gaming_device)]
        for emt in extra_edges: edge_list.append(emt)
        extra_nodes = [PLC1, PLC2, PLC3, PLC4, Air_condition, TV, Gaming_device, Desktop_PC]
        for emt in extra_nodes: node_list.append(emt)
...
def show_graph(nodes, edges, title):
    G = nx.Graph()
    ...
    nx.draw_networkx_labels(G, pos, mapping, font_size=8, font_color=(0, 0, 0))